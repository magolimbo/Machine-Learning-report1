...
file_path = r"C:\Users\Dell\Desktop\Git\Machine-Learning-report1\Movies_DS_test.xls"
doc = xlrd.open_workbook(file_path).sheet_by_index(0)

# Extract attribute names
attributeNames = doc.row_values(0, 2, 11)
...
gross1 = doc.col_values(6, 2, 636)
gross_name = sorted(set(gross1))   # set because it deletes the duplicates
grossDict = dict(zip(gross_name, range(10)))


# Extract vector y, convert to NumPy array
y_mpaa = np.array([mpaaDict[value] for value in mpaa])
y_genre = np.array([genreDict[value] for value in genre])
y_gross = np.array([grossDict[value] for value in gross1])

# Create a dataframe from the data
data = pd.DataFrame({'MPAA_Rating': y_mpaa, 'genre': y_genre, 'Budget': doc.col_values(4, 2, 636),
                     'Gross': doc.col_values(5, 2, 636), 'Gross1': y_gross, 'release_date': doc.col_values(7, 2, 636),
                    'runtime': doc.col_values(8, 2, 636), 'rating': doc.col_values(9, 2, 636), 'rating_count': doc.col_values(10, 2, 636)})
...
X = data[['MPAA_Rating', 'genre', 'Budget', 'Gross', 'Gross1',
          'release_date', 'runtime', 'rating', 'rating_count']].values
y_gross = data['Gross1'].values


# Don't know if this is needed
M = len(attributeNames)
N_gross = len(y_gross)
C_gross = len(gross_name)

grossNames = [gros for gros in grossDict]

# Select subset of digits classes to be inspected
class_mask = np.zeros(N_gross).astype(bool)
